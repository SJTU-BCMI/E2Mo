...
def train_one_epoch_EYE(
    ...
    for data_loader, ch_names in zip(data_loader_list, ch_names_list):
        ...
        for step, (batch) in enumerate(
            ...
            batch[0] = batch[0].float().to(device, non_blocking=True)
...
def calculate_codebook_usage(
    data_loader, model, device, log_writer=None, epoch=None, args=None
):
    ...
    for step, (images) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        outputs_gather_list = [
            torch.zeros_like(outputs) for _ in range(utils.get_world_size())
        ]
    ...
    print(
        f"STAT:  {zero_cnt} tokens ({(zero_cnt / codebook_num) * 100}%) never are used in this codebook."
    )
