...
from sklearn.metrics import accuracy_score, balanced_accuracy_score, f1_score, cohen_kappa_score, precision_recall_curve, auc, roc_auc_score
...
class SmoothedValue(object):
    ...
    def synchronize_between_processes(self):
        ...
        if self.type:
            if not is_dist_avail_and_initialized():
                return
            dist.barrier()
            self.deque = self.deque.cuda()
            global_tensor_list = [torch.zeros_like(self.deque,) for _ in range(dist.get_world_size())]

            dist.all_gather(global_tensor_list, self.deque)
            self.deque = global_tensor_list
            pass
        else:
            if not is_dist_avail_and_initialized():
                return
    ...
    def global_avg(self):
        if self.type:
            return torch.cat(self.deque,dim=0).cpu().numpy().tolist()
        else:
...
def auto_load_model_all_train(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None):
    output_dir = Path(args.output_dir)
...
def build_pretraining_dataset_depression(datasets: list, time_window: list, stride_size=200, start_percentage=0, end_percentage=1):
    shock_dataset_list = []
    ch_names_list = []
    for dataset_list, window_size in zip(datasets, time_window):
        dataset = ShockDataset([Path(file_path) for file_path in dataset_list], window_size * 200, stride_size, start_percentage, end_percentage)
...
class BCILoader(torch.utils.data.Dataset):
    def __init__(self, root, files, sampling_rate=200):
        self.root = root
        self.files = files
        self.default_rate = 200
    ...
    def __getitem__(self, index):
        ...
        # 256 * 5 -> 1000, from 256Hz to ?
        if self.sampling_rate != self.default_rate:
            X = resample(X, 3 * self.sampling_rate, axis=-1)
        # X = X / (
        #     np.quantile(np.abs(X), q=0.95, method="linear", axis=-1, keepdims=True)
        #     + 1e-8
        # )
...
class nEEGsaLoader(torch.utils.data.Dataset):
    def __init__(self, root, files, sampling_rate=200):
        self.root = root
        self.files = files
        self.default_rate = 200
...
def prepare_TUEV_dataset(root):
    ...
    #     os.path.join(
    #         root, "processed_train"), val_files
    # )
    test_dataset = TUEVLoader(
        os.path.join(
...
def prepare_BCI_dataset(root):
    ...
    # prepare training and test data loader
    train_dataset = BCILoader(
        os.path.join(
            root, "train"), train_files
    )
    test_dataset = BCILoader(
        os.path.join(
            root, "test"), test_files
    )
    val_dataset = BCILoader(
        os.path.join(
            root, "train"), val_files
    )
    print(len(train_files), len(val_files), len(test_files))
    return train_dataset, test_dataset, val_dataset

def prepare_nEEGsa_dataset(root):
    # set random seed
    seed = 12345
    np.random.seed(seed)
    ...
    # prepare training and test data loader
    train_dataset = nEEGsaLoader(os.path.join(root, "train"), train_files)
    test_dataset = nEEGsaLoader(os.path.join(root, "test"), test_files)
    val_dataset = nEEGsaLoader(os.path.join(root, "val"), val_files)
    print(len(train_files), len(val_files), len(test_files))
    return train_dataset, test_dataset, val_dataset

def prepare_CHBMIT_dataset(root):
    # set random seed
    seed = 12345
    ...
    # prepare training and test data loader
    train_dataset = CHBMITLoader(os.path.join(root, "train"),
                     train_files)
    test_dataset = CHBMITLoader(os.path.join(root, "test"),
                     test_files)
    val_dataset = CHBMITLoader(os.path.join(root, "val"), val_files)
...
def get_metrics(output, target, metrics, is_binary):
    ...
    #     if metric == 'acc':
    #         results['acc'] = accuracy_score(target, output)
    #     elif metric == 'balanced acc':
    #         results['acc'] = balanced_accuracy_score(target, output)
    #     elif metric == 'f1':
    #         results['f1'] = f1_score(target, output, average='weighted')
    #     elif metric == 'aucpr':
    #         precision, recall, _thresholds = precision_recall_curve(target, output)
    #         results['aucpr'] = auc(recall, precision)
    #     elif metric == 'auroc':
    #         results['auroc'] = roc_auc_score(target, output)
    #     elif metric == 'kappa':
    #         results['kappa'] = cohen_kappa_score(target, output)
    # return results
    if is_binary:
...
def get_metrics_regression(output, target, metrics):
    results = {}
    output = output.flatten()
    if "MSE" in metrics:
        results["MSE"] = np.mean((target - output) ** 2)
    if "RMSE" in metrics:
        results["RMSE"] = np.sqrt(np.mean((target - output) ** 2))
    if "MAE" in metrics:
        results["MAE"] = np.mean(np.abs(target - output))
    if "Rsquared" in metrics:
        mean_y_true = np.mean(target)
        sst = np.sum((target - mean_y_true) ** 2)  # 总平方和
        ssr = np.sum((target - output) ** 2)  # 残差平方和
        r_squared = 1 - (ssr / sst)
