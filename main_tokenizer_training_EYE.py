...
from engine_for_vqkd import evaluate, train_one_epoch, calculate_codebook_usage,train_one_epoch_EYE,evaluate_EYE
...
def get_args():
    parser = argparse.ArgumentParser('EEMo EYE tokenizer pre-training script', add_help=False)
...
def main(args):
    ...
    train_dataset,ch_name = get_pretrain_dataset("A_depression_EYE_woonehot", "train")
    dataset_train_list = []
    train_ch_names_list = []
    dataset_train_list.append(train_dataset)
    train_ch_names_list.append(ch_name)
    dataset_train_list = [train_dataset]
    train_ch_names_list = [ch_name]
    if args.disable_eval:
        dataset_val_list = None
    else:
        val_dataset, _ = get_pretrain_dataset("A_depression_EYE_woonehot", "val")
        dataset_val_list = [val_dataset]
        val_ch_names_list = [ch_name]
    ...
    # if not args.resume:
    #     pretrained_vqkd_path = "/data/jiangweibang/result_yhl/labram/checkpoint/vqkd_labram/checkpoint-99.pth"
    #     # pretrained_vqkd_path = "/home/jiangweibang/LEM/checkpoint/vqkd/checkpoint-99.pth"
    #     # pretrained_vqkd_path = "/data/jiangweibang/result_yhl/labram/checkpoint/vqkd_39/checkpoint-99.pth"
    #     checkpoint = torch.load(pretrained_vqkd_path, map_location='cpu')
    #     model_without_ddp.load_state_dict(checkpoint['model'])

    if args.eval:
        test_stats = evaluate_EYE(data_loader_val, model, device, log_writer, 0, args=args)
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_EYE(
            model, 
            data_loader_train_list,
            optimizer, 
            device, 
        ...
        if data_loader_val_list is not None:
            test_stats = evaluate_EYE(data_loader_val_list, model, device, log_writer, epoch, ch_names_list=val_ch_names_list, args=args)
